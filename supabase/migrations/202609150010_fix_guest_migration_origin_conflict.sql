-- Qualify the generation-origin upsert through its primary-key constraint.
-- The function also returns a field named diagram_id, so the earlier
-- ON CONFLICT (diagram_id) form is ambiguous inside PL/pgSQL.
...
begin
  ...
  if generation_origin is not null then
    ...
    on conflict on constraint diagram_generation_origins_pkey do nothing;
