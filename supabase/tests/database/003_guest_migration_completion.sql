...
select lives_ok($test$
  select * from public.migrate_guest_architecture_complete(
    ...
    (select public.canonical_jsonb_sha256(jsonb_build_object('ir',ir,'presentation',presentation,'diagram',diagram)) from complete_fixture),
    'Complete migration', (select ir from complete_fixture), (select public.canonical_jsonb_sha256(ir) from complete_fixture),
    (select presentation from complete_fixture), (select public.canonical_jsonb_sha256(presentation) from complete_fixture),
    (select diagram from complete_fixture), (select public.canonical_jsonb_sha256(diagram) from complete_fixture),
    'manual-edit', '1.1.0', '1.0.0', '# Migrated document', (select origin from complete_fixture),
    (select public.canonical_jsonb_sha256(origin) from complete_fixture), '14141414-1414-4414-8414-141414141414')
...
select is((select artifact_checksum from public.diagram_generation_origins where diagram_id='15151515-1515-4515-8515-151515151515'), (select public.canonical_jsonb_sha256(origin) from complete_fixture), 'signed generation origin is retained');
select lives_ok($test$
  select * from public.migrate_guest_architecture_complete(
    '16161616-1616-4616-8616-161616161616',
    (select public.canonical_jsonb_sha256(jsonb_build_object('ir',ir,'presentation',presentation,'diagram',diagram)) from complete_fixture),
