...
export const patternDefinitionSchema = z.object({
  ...
  for (const field of ["knownRisks", "questions", "matchTerms"] as const) {
    if (new Set(definition[field].map((item) => item.toLowerCase())).size !== definition[field].length) {
...
const templateMetadata: Record<string, Pick<PatternDefinition, "family" | "knownRisks" | "questions" | "matchTerms" | "priority">> = {
  ...
  "ai-rag": { family: "ai", knownRisks: ["Retrieved context can be stale, irrelevant, or expose unauthorized sources."], questions: ["How are source permissions and answer citations verified?"], matchTerms: ["rag", "retrieval", "llm", "ai", "ai assistant"], priority: 70 },
  ecommerce: { family: "commerce", knownRisks: ["Inventory and payment state can diverge across partial failures."], questions: ["What consistency guarantees apply to checkout and inventory?"], matchTerms: ["ecommerce", "e-commerce", "e commerce", "checkout", "shopping", "inventory"], priority: 70 },
  ...
  realtime: { family: "realtime", knownRisks: ["Connection fan-out and reconnect storms can overwhelm stateful services."], questions: ["What latency, presence, and reconnect guarantees are required?"], matchTerms: ["realtime", "real-time", "real time", "websocket", "collaboration"], priority: 65 },
...
function buildPatternRegistry() {
  ...
  return Object.freeze(definitions.map((definition) => Object.freeze({
    ...definition,
    requiredComponents: Object.freeze([...definition.requiredComponents]),
    optionalComponents: Object.freeze([...definition.optionalComponents]),
    knownRisks: Object.freeze([...definition.knownRisks]),
    questions: Object.freeze([...definition.questions]),
    matchTerms: Object.freeze([...definition.matchTerms]),
  }))) as readonly PatternDefinition[];
}
...
function termPattern(term: string) {
  ...
  const pluralSuffix = /[a-z0-9]$/.test(normalized) && !normalized.endsWith("s") ? "(?:s|es)?" : "";
  return new RegExp(`(^|[^\\p{L}\\p{N}])(${escapeRegExp(normalized)}${pluralSuffix})(?=$|[^\\p{L}\\p{N}])`, "giu");
}

function termMatches(searchable: string, term: string) {
  const matches = [...searchable.matchAll(termPattern(term))];
  let positive = false;
  let negated = false;
  for (const match of matches) {
    const termStart = (match.index ?? 0) + match[1].length;
    const clausePrefix = searchable.slice(Math.max(0, termStart - 100), termStart).split(/[,.!?;:\n]/).at(-1)?.trim() ?? "";
    const negation = /(?:^|\s)(?:no|not|without|avoid|avoiding|exclude|excluding|never|don['’]t|doesn['’]t|do not|does not|must not|should not)(?:\s+[\p{L}\p{N}'’-]+){0,6}\s*$/iu.exec(clausePrefix);
    const explicitlyNegated = Boolean(negation && !/\bnot\s+only\b/i.test(negation[0]));
    if (explicitlyNegated) negated = true;
    else positive = true;
  }
...
export function matchArchitecturePatterns(input: GenerationRequest): PatternMatch[] {
  ...
  const searchable = `${request.prompt} ${request.productType ?? ""}`.toLowerCase();
  const inferred = patternRegistry.flatMap((pattern) => {
    const matchedTerms = pattern.matchTerms.filter((term) => termMatches(searchable, term).positive);
  ...
  const explicitMatch: PatternMatch = {
    pattern: explicit,
    matchedTerms: explicit.matchTerms.filter((term) => termMatches(searchable, term).positive),
...
export const ruleDefinitionSchema = z.object({
  ...
  for (const field of ["matchTerms", "proposedComponents", "proposedRequirements"] as const) {
    if (new Set(definition[field].map((item) => item.toLowerCase())).size !== definition[field].length) {
      ctx.addIssue({ code: "custom", path: [field], message: `${field} must be unique.` });
    }
  }
...
function buildRuleRegistry() {
  const definitions = ruleInputs.map((item) => ruleDefinitionSchema.parse({ schemaVersion: RULE_DEFINITION_VERSION, ...item }));
  if (new Set(definitions.map((item) => item.id)).size !== definitions.length) throw new Error("Rule IDs must be unique.");
  return Object.freeze(definitions.map((definition) => Object.freeze({
    ...definition,
    matchTerms: Object.freeze([...definition.matchTerms]),
    proposedComponents: Object.freeze([...definition.proposedComponents]),
    proposedRequirements: Object.freeze([...definition.proposedRequirements]),
  }))) as readonly RuleDefinition[];
}
...
export const ruleProposalSchema = z.object({
  ...
  ruleId: z.string().max(80).regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/),
  ...
  if (!ruleRegistry.some((rule) => rule.id === proposal.ruleId)) {
    ctx.addIssue({ code: "custom", path: ["ruleId"], message: `Unknown architecture rule ${proposal.ruleId}.` });
  }
  for (const field of ["matchedTerms", "proposedComponents", "proposedRequirements"] as const) {
    if (new Set(proposal[field].map((item) => item.toLowerCase())).size !== proposal[field].length) {
      ctx.addIssue({ code: "custom", path: [field], message: `${field} must be unique.` });
    }
  }
  if (proposal.status === "not-matched" && (proposal.matchedTerms.length > 0 || proposal.proposedComponents.length > 0 || proposal.proposedRequirements.length > 0 || proposal.conflict)) {
    ctx.addIssue({ code: "custom", message: "Not-matched rules cannot contain matches, proposals, or conflicts." });
  }
  if (proposal.status === "matched" && (proposal.matchedTerms.length === 0 || proposal.conflict)) {
    ctx.addIssue({ code: "custom", message: "Matched rules require matched terms and cannot contain a conflict." });
  }
  if (proposal.status === "conflicting" && (proposal.matchedTerms.length === 0 || !proposal.conflict || proposal.proposedComponents.length > 0)) {
    ctx.addIssue({ code: "custom", message: "Conflicting rules require matched terms and a conflict, and cannot propose components." });
  }
});
...
export function evaluateArchitectureRules(options: {
  ...
  return ruleRegistry.map((rule) => {
    const matches = rule.matchTerms.map((term) => ({ term, ...termMatches(searchable, term) }));
    const matchedTerms = matches.filter((match) => match.positive).map((match) => match.term);
    const negatedTerms = matches.filter((match) => match.negated).map((match) => match.term);
    if (matchedTerms.length === 0 && negatedTerms.length === 0) {
    ...
    const conflict = negatedTerms.length > 0
      ? `Explicit language excludes ${negatedTerms.join(", ")}; the proposal was not applied.`
      : explicitRuleConflict(rule, request, searchable);
    if (conflict) {
      return ruleProposalSchema.parse({ schemaVersion: RULE_EVALUATION_VERSION, ruleId: rule.id, status: "conflicting", matchedTerms: [...new Set([...matchedTerms, ...negatedTerms])], rationale: "A matched suggestion conflicts with an explicit constraint and was not applied.", conflict, proposedComponents: [], proposedRequirements: rule.proposedRequirements });
    }
