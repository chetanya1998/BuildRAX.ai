...
export async function readGenerationStages(jobId: string) {
  const { data, error } = await adminClient().from("generation_job_stages").select("stage,payload").eq("job_id", jobId);
...
export async function listQueuedGenerationJobs(limit = 2) {
  const boundedLimit = Math.max(1, Math.min(limit, 10));
  const { data, error } = await adminClient().rpc("list_queued_generation_jobs", { batch_limit: boundedLimit });
...
export async function completeGenerationJob(options: { workerId: string; jobId: string; runVersion: number; result: unknown }) {
  ...
  const { error } = await adminClient().rpc("complete_generation_job", {
    worker_id: options.workerId,
    target_job: options.jobId,
    expected_run_version: options.runVersion,
    result_payload: options.result,
...
export async function failGenerationJob(options: { workerId: string; jobId: string; runVersion: number; errorClass: string; message: string }) {
  await adminClient().rpc("fail_generation_job", {
    worker_id: options.workerId,
    target_job: options.jobId,
    expected_run_version: options.runVersion,
    failure_class: options.errorClass,
    failure_message: options.message,
  });
}
