...
describe("architecture AI contracts", () => {
  it("returns a schema-valid editable diagram", async () => {
    const result = await new MockArchitectureProvider().generate({ prompt: "Build a multi-tenant SaaS product with background jobs" });
    expect(architectureIRSchema.safeParse(result.output).success).toBe(true);
    const diagram = compileArchitectureIR(result.output);
  ...
  it("binds review and documentation to the diagram version", async () => {
    const result = await new MockArchitectureProvider().generate({ prompt: "Build an ecommerce backend with a database" });
    const diagram = compileArchitectureIR(result.output);
