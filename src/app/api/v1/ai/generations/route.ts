...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ diagram: result.diagram, meta: { requestId, durationMs: Date.now() - startedAt, provider: result.provider, attempts: result.attempts } }, { headers: { "cache-control": "no-store", "x-request-id": requestId } });
  } catch (error) {
    void recordGenerationRun({ requestId, provider: run?.provider ?? (process.env.OPENAI_API_KEY ? "openai" : "mock"), model: run?.model ?? (process.env.OPENAI_MODEL || (process.env.OPENAI_API_KEY ? "gpt-5.4-mini-2026-03-17" : "deterministic-fixture-v1")), status: "failed", durationMs: Date.now() - startedAt, promptVersion: run?.promptVersion ?? "architecture-v1", attempts: run?.attempts ?? 1, errorClass: classifyAIError(error) });
